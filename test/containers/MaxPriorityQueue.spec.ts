...
describe('MaxPriorityLinkedQueue', () => {
    let empty: Queue<number>
    let tiny: Queue<number>

    beforeEach(() => {
        empty = newMaxPQ<number>(numberComparator)
        tiny = newMaxPQ<number>(numberComparator).enqueue(2).enqueue(1).enqueue(3)
    })
    ...
    describe('Queue as a Container', () => {
        ...
        it('should work for not-empty queue', () => {
            ...
            const collection = newMaxPQ<number>(numberComparator)
                .enqueue(2)
                .enqueue(1)
                .enqueue(3)
                .enqueue(5)
